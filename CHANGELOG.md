...
### Added
- Collection element guard via `each` — iterates over the guarded value, yields a nested Guard for each element; in hard mode raises on first element violation, in soft mode collects all child errors prefixed with element index
